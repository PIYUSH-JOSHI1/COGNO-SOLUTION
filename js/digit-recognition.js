...
function displayLabel(data) {
	...
	predictedItem = maxIndex;
	$(".prediction-text").html("Predicting you draw <b>" + maxIndex + "</b> with <b>" + Math.trunc(max * 100) + "%</b> confidence")

	predictNumber = predictedItem;
}


let drawQuestions = ["ZERO", "SEVEN", "FIVE", "FOUR", "ONE", "TWO", "THREE", "EIGHT"];
let drawAns = [0, 7, 5, 4, 1, 2, 3, 8]
...
function firstG() {
	...
	if (questionCounter < 4) {
		...
		document.getElementById("questionNumber").innerHTML = "Draw Number " + drawQuestions[random];
...
function nextQuestion() {
	...
	if (predictNumber == drawAns[random]) {
		currScore += 20;
		alert("CORRECT")
		firstG();
	}
